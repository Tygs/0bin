...
- plus de gens pourront vous aider sur les forums ;
- votre hébergeur propose sûrement Apache ;
- la syntaxe des fichiers de configuration est familière pour beaucoup.

Une installation Apache est aussi beaucoup plus solide et sécurisée qu'une
...
l'installation du module Apache mod_wsgi. Si vous ne savez pas comment faire,
...
fichier Python qui crée l'application qu'Apache va utiliser pour lancer le
processus Python : ::
...
Vous pouvez bien sûr créer le vôtre, puisque la fonction `get_app` et le seul
